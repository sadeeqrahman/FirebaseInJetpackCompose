...
@Composable
fun LoginButton(username: String, password: String, onClick: (String, String) -> Unit) {
    ...
    when (loginResult) {
        is AuthResult.Success -> {
            val user = (loginResult as AuthResult.Success).user
            context.startActivity(Intent(context,HomeActivity::class.java))
        }

        is AuthResult.Error -> {
            val exception = (loginResult as AuthResult.Error).exception
